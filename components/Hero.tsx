...
export function Hero({ sidebarMinimized = false }: HeroProps) {
  return (
    <section className="relative min-h-[280px] md:min-h-[360px] lg:h-[398px] overflow-hidden rounded-none bg-[#4847ae] backdrop-blur-[60px] w-full flex flex-col lg:block">
      {/* Background Image - hidden on mobile, visible on desktop */}
      <div className={`hidden lg:block absolute top-[20.6%] bottom-[20.6%] rounded-[14px] overflow-hidden z-0 transition-all duration-300 ${sidebarMinimized
      ...
      <Button
        variant="outline"
        size="sm"
        className={`absolute top-3 md:top-4 lg:top-[9.42%] bg-white/10 border border-white/5 backdrop-blur-sm text-white hover:bg-white/30 hover:border-white/20 rounded-full px-2 md:px-3 py-1.5 md:py-2 h-auto min-h-[36px] md:min-h-[40px] transition-all duration-300 z-20 flex items-center gap-[5px] ${sidebarMinimized
      ...
      <div className={`relative lg:absolute w-full md:w-[90%] max-w-[346px] lg:w-[346px] mx-auto lg:mx-0 mt-14 lg:mt-0 lg:top-[84px] flex flex-col gap-3 md:gap-4 lg:gap-[21px] items-start justify-start z-10 transition-all duration-300 px-4 md:px-6 lg:px-0 ${sidebarMinimized
